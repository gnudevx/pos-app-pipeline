...
│   │   ├── dev-agent-core.md
│   │   ├── dev-agent-gemini.md
│   │   └── tester-agent-gemini.md
│   └── react.js
│
├── ci/
│   ├── merge/
│   │   └── merge_coordinator.py    # Điều phối merge sau khi pass test
│   ├── repair/
│   │   ├── fix_agent.py            # AI agent tự sửa lỗi build/test
│   │   └── self_healing.py         # Cơ chế self-healing pipeline
│   ├── runtime/
│   │   ├── build_verifier.py       # Kiểm tra build thành công
│   │   └── validate_contracts.py   # Kiểm tra contract giữa các service
│   └── ci.md
│
├── context/
│   └── claude.md
...
├── core/
│   ├── agents/
│   │   ├── architect_agent.py      # Sinh kiến trúc hệ thống từ requirements
│   │   ├── planner_agent.py        # Chia Epic/User Story thành các task thực thi
│   │   ├── requirement_agent.py    # Phân tích và chuẩn hóa yêu cầu đầu vào
│   │   ├── dev_agent.py            # Sinh code cho từng task
│   │   ├── tester_agent.py         # Sinh test và xác thực kết quả
│   │   └── knowledge_graph_agent.py   # Xây dựng Knowledge Graph phục vụ planning
│   │
│   ├── contracts/
│   │   ├── parser.py
│   │   ├── indexer.py
...
│   │
│   ├── infra/
│   │   ├── ai_client.py            # Wrapper gọi Gemini/OpenAI và quản lý API key
│   │   ├── git_ops.py              # Các thao tác Git branch/commit/merge
│   │   ├── slot_injector.py        # Chèn code AI sinh ra vào đúng vị trí scaffold
│   │   └── smart_scaffold.py       # Tạo scaffold dự án và đồng bộ cấu trúc
│   │
│   ├── pipeline/
│   │   ├── jira_sync.py
│   │   ├── routers.py
...
│   │
│   ├── orchestrator_v2.py          # Entry point điều phối toàn bộ pipeline
│   ├── start_integration.py
│   ├── state_manager.py            # Quản lý trạng thái chạy của workflow
│   └── config.py
│
├── docs/
│   ├── contracts/                  # Contract sinh ra cho từng task
│   ├── structure_plans/            # Kế hoạch triển khai chi tiết
│   ├── architecture.json           # Kiến trúc hệ thống
│   ├── dependency_graph.json
│   ├── knowledge_graph.json        # Knowledge Graph được sinh tự động
│   ├── materialized_tasks.json
│   ├── stories.json
│   ├── tasks.json                  # Danh sách task đã materialize
...
│   ├── mock_agents.py
│   ├── mock_agents_v2.py
│   ├── mock_dev_agent_v2.py
│   ├── mock_tester_agent_v2.py
...
flowchart TD
    ...
    subgraph P5["Phase 5 — Sprint Execution Loop [orchestrator_v2.py]"]
        ...
        P --> R{next task?}
        R -- yes --> L
        R -- no --> S[End Sprint State]
    end

    %% Ép cấu trúc Phase 6 và Phase 7 xuống dưới Phase 5 theo hàng dọc
    S --> P6

    subgraph P6["Phase 6 — CI/CD [GitHub Actions]"]
        U_CI[lint · build check\nDeploy → Staging]
    end
    ...
    %% Định nghĩa màu sắc dịu nhẹ, tương thích tốt với cả Light/Dark Mode
    %% Chữ (color) được ép màu xám tối/sáng tùy biến hoặc giữ mặc định an toàn
    style P1 fill:#2e3b2e,stroke:#4CAF50,stroke-width:2px,color:#fff
    style P2 fill:#243342,stroke:#2196F3,stroke-width:2px,color:#fff
    style P3 fill:#3d3021,stroke:#FF9800,stroke-width:2px,color:#fff
    style P3B fill:#33253d,stroke:#9C27B0,stroke-width:2px,color:#fff
    style P4 fill:#25283d,stroke:#3F51B5,stroke-width:2px,color:#fff
    style P5 fill:#3d252b,stroke:#E91E63,stroke-width:2px,color:#fff
    style P6 fill:#1f3633,stroke:#009688,stroke-width:2px,color:#fff
    style P7 fill:#2d3621,stroke:#8BC34A,stroke-width:2px,color:#fff
```
...
https://github.com/gnudevx/pos-app-test_v2 
src/
├── backend/
│   └── app/
