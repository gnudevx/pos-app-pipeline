"""
Dependency Graph Builder  [FIXED v2]
...
FIXES:
  [FIX-D1] Thêm validation: warn khi backend phụ thuộc frontend (cross-layer dep)
  [FIX-D2] save_graph() trả về graph để caller có thể dùng lại, không cần load lại file
"""
...
def build_dependency_graph(architecture: dict) -> dict:
    ...
    # Build component map for cross-layer validation [FIX-D1]
    component_map = {svc["task_id"]: svc.get("component", "unknown")
                     for svc in services if "task_id" in svc}
    ...
    for svc in services:
        ...
        for dep in svc.get("depends_on", []):
            if dep in nodes:
                ...
                # [FIX-D1] Warn cross-layer: backend deps on frontend is always wrong
                src_comp = component_map.get(task_id, "")
                dep_comp = component_map.get(dep, "")
                if src_comp == "backend" and dep_comp == "frontend":
                    print(
                        f"      [dep-graph] WARN: cross-layer dep {task_id}({src_comp}) "
                        f"→ {dep}({dep_comp}) — backend should NOT depend on frontend"
                    )
...
def save_graph(graph: dict, path: str = "docs/dependency_graph.json"):
    ...
    # [FIX-D2] Không thay đổi gì ngoài việc trả về graph
    return graph
